...
func TestUsernameIncorect(t *testing.T) {
	...
	for _, fixture := range fixtures {
		...
		if err.Error() == "Username must not be less than 5 characters" {
			g.Expect(err.Error()).To(Equal("Username must not be less than 5 characters"))
		} else if err.Error() == "Username must contain lowercase character and numbers" {
			g.Expect(err.Error()).To(Equal("Username must contain lowercase character and numbers"))
		}
		// g.Expect(err.Error()).To(Equal("Username must not be less than 5 characters"))
	}
}
